module class_mesh
  ...
  contains
    function mesh_init(nx, ny, nz, uInput, vInput) result(val)
        ...
        do i = 0, nx - 1
          NewFace = face_init(val%faces_(dimX)%tail_, val%coord_%x_(i), &
            [val%coord_%y_(j), val%coord_%y_(j + 1)], [i, j], val%coord_%l0_, &
             val%coord_%x_, val%coord_%y_, val%coord_%l0_, nx, ny, nz, uInput, vInput, dimX, null_face)
      ...
      do i = 0, nx/2 - 1
        NewFace = face_init(val%faces_(dimY)%tail_, val%coord_%y_(j), &
          [val%coord_%x_(i), val%coord_%x_(i + 1)], [i, j], val%coord_%l0_, &
           val%coord_%x_, val%coord_%y_, val%coord_%l0_, nx, ny, nz, uInput, vInput, dimY, null_face)
    ...
    subroutine mesh_sync(self, nx, ny, nz, uInput, vInput)
    ! synchronize the old mesh with new mesh to prepare for next time step
      ...
      do while(associated(MeshColumn))
        ...
        if (MeshColumn%OtherMesh_%l_ < MeshColumn%l_) then
          ...
          call ColumnList_Coarsen(self%columns_, self%faces_, MeshColumn, &
            self%coord_%x_, self%coord_%y_, self%coord_%l0_, nx, ny, nz, uInput, vInput)
        ...
        ! if the new mesh is finer than the current mesh, refine all the leaves
          ...
          do while (associated(StackColumns%head_))
            ...
            call ColumnList_refine(self%columns_, StackColumn, self%faces_, &
              self%coord_%x_, self%coord_%y_, self%coord_%l0_, nx, ny, nz, uInput, vInput)
            do i = 0, 1
              do j = 0, 1
