module class_column
  ...
  contains
    ...
    subroutine column_FaceCoarsen(CoarseColumn, Faces, NewFaces, x, q, x0, y0, l0, nx, ny, nz, uInput, vInput)
      ...
      enddo
      q(1:, 1:) = q(1:, 1:)/dA
      call column_FaceLevelAdjust(CoarseColumn, Faces, NewFaces, x0, y0, l0, nx, ny, nz, uInput, vInput)
    end subroutine column_FaceCoarsen

    subroutine column_FaceLevelAdjust(CoarseColumn, Faces, NewFaces, x, y, l0, nx, ny, nz, uInput, vInput)
      use class_AMRTypes,                 only: FaceList, FaceConnect, face
        ...
        do indx = 1, 2
          ...
          do while( associated(MeshFace) )
            ...
            call Face_Coarsen(MeshFace, min(0, max(CoarseColumn%l_, AdjColumn%l_) &
             - MeshFace%prop_%l_), x, y, l0, nx, ny, nz, uInput, vInput, dim)
    ...
    subroutine column_refineface(self, Lscale, i00, x0, dx, Faces, NewFaces, x, y, l0, nx, ny, nz, uInput, vInput)
      ...
      do j = 0, Lscale
        do i = 0, Lscale - 1
          NewFace = face_init(prev, x0(dimX) + (i + 1)*dx(dimX), x0(dimY) + [j, j + 1]*dx(dimY), &
            i00(:) + [i + 1, j], Ltarget, x, y, l0, nx, ny, nz, uInput, vInput, dimX, next)
      ...
      do j = 0, Lscale - 1
        do i = 0, Lscale
          NewFace = face_init(prev, x0(dimY) + (j + 1)*dx(dimY), x0(dimX) + [i, i + 1]*dx(dimX), &
            i00(:) + [i, j + 1], Ltarget, x, y, l0, nx, ny, nz, uInput, vInput, dimY, next)
      ...
      do dim = dimX, dimY
        do indx = 1, 2
          ...
          do i = 1, self%prop_%faces_(dim, indx)%n_
            ...
            ! generate new faces
            call FaceList_refine(Faces(dim), MeshFace, dim, dL, x, y, l0, nx, ny, nz, uInput, vInput)
    ...
    subroutine ColumnList_Coarsen(self, Faces, FineColumn, x, y, l0, nx, ny, nz, uInput, vInput)
      ...
      call column_FaceCoarsen(CoarseColumn, Faces, CoarseColumn%prop_%Faces_, &
        CoarseColumn%prop_%x_, CoarseColumn%prop_%q_, x, y, l0, nx, ny, nz, uInput, vInput)
    ...
    subroutine ColumnList_refine(self, CoarseColumn, Faces, x, y, l0, nx, ny, nz, uInput, vInput)
      use class_kind,                          only: dp
      use class_AMRTypes,                      only: face, FaceList, FaceConnect
      use class_parameter,                     only: dimX, dimY
      type(ColumnList),           intent(inout)   :: self
      ...
      call column_refineface(CoarseColumn, Lscale, i00, x0, dx, Faces, NewFaces, x, y, l0, nx, ny, nz, uInput, vInput)
    ...
    theta_r, theta_c, dt, x, y, dxMax, uInput, vInput, is_init, icycle)
      use class_kind,      only: dp
      use class_AMRTypes,  only: FaceList
      use class_parameter, only: dimX, dimY
      type(ColumnList),          intent(inout) :: self
      type(FaceList),            intent(inout) :: Faces(dimX:)
      integer,                   intent(in)    :: l0, MaxRef, ntracer, nx, ny, nz
      real(dp),                  intent(in)    :: theta_r, theta_c, x(0:), y(0:), dxMax(dimX:), dt
      real(dp),                  intent(in)    :: uInput(1:, 1:, 1:), vInput(1:, 0:, 1:)      
        ...
        do while(associated(MeshColumn))
          ...
          if ((MeshColumn%prop_%theta_ < theta_c) .and. (MeshColumn%l_ > l0)) then
            ...
            do l = 1, MeshColumn%l_ - l0
              ...
              enddo
            enddo
            call ColumnList_Coarsen(self, Faces, MeshColumn, x, y, l0, nx, ny, nz, uInput, vInput)
