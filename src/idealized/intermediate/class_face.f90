...
contains
  function Face_init(prev, x, y, i, l, nz, TestName, t, dt, beta, dim, next) result(val)
  ...
  subroutine Face_coarsen(self, l, nz, TestName, t, dt, beta, dim)
    use class_setupwind,          only: setup_ideal
  ...
  subroutine FaceList_refine(self, MeshFace, dim, l, nz, TestName, t, dt, beta)
    use class_setupwind,                 only: setup_ideal
    ...
    do i = 1, Lscale - 1
      NewFace = face_init(prev, MeshFace%prop_%x_, MeshFace%prop_%y_(1) + [i, i + 1]*dy, &
        MeshFace%prop_%i_(1:) + [i*(dim - 1), i*(2 - dim)], MeshFace%prop_%l_, nz, TestName, t, dt, beta, dim, next)
    ...
    if (associated(MeshFace%pole_)) then
      ...
      do i = 1, Lscale - 1
        NewFace = face_init(prev, MeshFace%pole_%prop_%x_, MeshFace%pole_%prop_%y_(1) + &
          ...
          MeshFace%pole_%prop_%l_, nz, TestName, t, dt, beta, dim, next)
