...
contains
  subroutine netcdf_init(nx, ny, nz, ntracer, MaxRef)
    ...
    allocate(names(8 + ntracer + 1 + 2))
    if (allocated(units)) deallocate(units)
    allocate(units(8 + ntracer + 1 + 2))
    if (allocated(varid)) deallocate(varid)
    allocate(varid(ntracer + 8 + 1 + 2))
    ...
    names(10)%c = 'qX'
    names(11)%c = 'qY'
    do itracer = 9, ntracer + 1 + 8 + 2
      ! names(itracer)%c = 'q'
      units(itracer)%c = 'kg kg-1'
    enddo

    ! setup output netcdf
    call writenc_setup(nColumnsTotal, nz, 4, ntracer + 8 + 1 + 2, trim(adjustl(Tunit)), names, units)
    deallocate(names)
    deallocate(units)
    ...
    allocate(q(ntracer + 1 + 2, nz, nColumnsTotal))        
  ...
  subroutine netcdf_writevar(columns, t, dt, nz, ntracer)
    ...
    do i = 1, columns%n_
      ...
      q(2, :, i)        = MeshColumn%prop_%qinter_(1, :, dimX)
      q(3, :, i)        = MeshColumn%prop_%qinter_(1, :, dimY)
    ...
    it = it + 1
    call writenc_vars(ntracer + 8 + 1 + 2, columns%n_, nz)
